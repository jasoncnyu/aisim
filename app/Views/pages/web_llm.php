...
<div class="space-y-4">
    <div class="border-b border-slate-200 pb-3">
        <h2 class="text-2xl font-semibold">Tiny Web LLM Lab</h2>
        <p class="text-slate-500">Train a small next-token model directly in the browser and generate text.</p>
    </div>
    ...
    <div class="space-y-2">
        <section x-data="{ open: true }" class="rounded-xl border border-slate-200 bg-white">
            <button type="button" class="w-full px-4 py-3 text-left font-medium flex items-center justify-between" @click="open = !open">
                <span>1) What This Simulator Teaches</span>
            ...
            <div x-show="open" x-transition x-cloak class="px-4 pb-4 text-sm text-slate-700 space-y-2">
                <p>This page demonstrates the core mechanics of language modeling: tokenization, context windows, logits, softmax, and gradient-based updates.</p>
                <p>It is intentionally tiny and educational. You can inspect training loss and compare decoding strategies such as Greedy, Sampling, and Top-k.</p>
            </div>
        </section>

        <section x-data="{ open: false }" class="rounded-xl border border-slate-200 bg-white">
            <button type="button" class="w-full px-4 py-3 text-left font-medium flex items-center justify-between" @click="open = !open">
                <span>2) Model Design</span>
                <span class="text-slate-500" x-text="open ? '-' : '+'"></span>
            </button>
            <div x-show="open" x-transition x-cloak class="px-4 pb-4 text-sm text-slate-700 space-y-2">
                <p>Tokens are space-split words. The model averages token embeddings in the current context and projects them to vocabulary logits.</p>
                <p class="text-center">$$ h = \frac{1}{K}\sum E(t_i),\quad z = W^Th + b,\quad p = softmax(z) $$</p>
                <p>Training uses per-token cross-entropy with simple SGD updates.</p>
            </div>
        </section>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <div class="space-y-4 lg:col-span-8">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h3 class="mb-2 text-lg font-semibold">1) Training Corpus</h3>
...
You can modify the prompt if you want . However you must use only the words that appear in this training data . This is because the vocabulary used in this dataset is the entire vocabulary that this model knows .</textarea>
                <div class="mt-3 flex flex-wrap items-center gap-2 text-sm">
                    <span class="text-slate-500">Load demo:</span>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h3 class="mb-2 text-lg font-semibold">2) Hyperparameters</h3>
                <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
                    <label class="text-sm">Embedding Size
                    ...
                    <label class="text-sm">Context Length
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h3 class="mb-2 text-lg font-semibold">3) Training Run</h3>
                <div class="flex items-center gap-3">
                    <button id="trainBtn" class="rounded-lg bg-slate-900 px-3 py-1.5 text-sm text-white">Start Training</button>
                    <button id="trainStopBtn" class="rounded-lg border border-slate-300 px-3 py-1.5 text-sm">Stop</button>
        ...
        <div class="space-y-4 lg:col-span-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h3 class="mb-2 text-lg font-semibold">4) Text Generation</h3>
                <label class="text-sm">Prompt
                ...
                <div class="mt-3 grid grid-cols-2 gap-2">
                    ...
                    <label class="text-sm">Temperature
                ...
                <div class="mt-3 flex flex-wrap items-center gap-3 text-sm">
                    <label class="inline-flex items-center gap-2"><input class="h-4 w-4" type="radio" name="sampling" value="greedy" checked><span>Greedy</span></label>
                    <label class="inline-flex items-center gap-2"><input class="h-4 w-4" type="radio" name="sampling" value="sample"><span>Sampling</span></label>
                    <div class="inline-flex items-center gap-2">
                        <label class="inline-flex items-center gap-2"><input class="h-4 w-4" type="radio" name="sampling" value="topk"><span>Top-k</span></label>
                        <span id="topkWrap">
                            <input type="number" id="topkK" class="h-8 w-20 rounded-lg border border-slate-300 px-2 py-1 text-sm" value="5" min="1" max="50">
                        </span>
                    </div>
                </div>
                <button id="genBtn" class="mt-3 rounded-lg border border-slate-300 px-3 py-1.5 text-sm">Generate</button>
